...
int main(int argc, char *argv[])
{
    ...
    //////// The below way of declaring the str array is too vulnerable to bufferoverflow, alternative is to change to dynamically allocated size.///////////
    //char str[MAX_DATA_SIZE];
    //////// Below is dynamically allocated space for array str, much less vulnerable to buffer overflow than the above way previously initialized.///////////
    char* str = malloc(sizeof(char)*MAX_DATA_SIZE);
	...
	if (send(clntSockfd, "Connected!!!\n", 
        ...
        while(1) {
            ...
            recvBuff[numBytes] = '\0';
	    //////////// The below code is vulnerable to buffer overflow attack since it takes the input directly as the size it comes in even if it exceeds the buffer, to prevent this will input a flag/warning that is triggered if input ever exceeds size of buffer
	    //strcpy(str, recvBuff);

	    if (MAX_DATA_SIZE < strlen(recvBuff)){
	      printf("Warning! Buffer Overflow Warning!");
	      exit(1);
	    }
            strcpy(str, recvBuff);
